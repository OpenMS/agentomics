...
Feature Linker QT
=================
Link features across multiple LC-MS/MS runs using the QT (quality threshold)
...
Usage
-----
    python feature_linker_qt.py --inputs run1.featureXML run2.featureXML \
...
def link_features_qt(
    input_paths: List[str],
    output_path: str,
    rt_tol: float = 30.0,
    mz_tol: float = 0.01,
) -> int:
    """Link features across multiple runs using the QT algorithm.
...
def main(inputs, output, rt_tol, mz_tol):
    """Link features across multiple LC-MS/MS runs using QT clustering."""
    input_paths = list(inputs)
    if len(input_paths) < 2:
        click.echo("Error: At least two input files are required.", err=True)
        sys.exit(1)

    count = link_features_qt(input_paths, output, rt_tol=rt_tol, mz_tol=mz_tol)
