"""Tests for protein_inference_bayesian."""
...
def _create_peptide_protein_idxml(path):
    ...
    All hits have target_decoy meta values required by BayesianProteinInference.
    """
    ...
    prot1.setScore(0.0)
    prot1.setMetaValue("target_decoy", "target")

    prot2 = oms.ProteinHit()
    prot2.setAccession("protein2")
    prot2.setScore(0.0)
    prot2.setMetaValue("target_decoy", "target")
...
class TestProteinInferenceBayesian:

    def test_infer_proteins_produces_output(self):
        """Test that Bayesian inference writes an output file."""
        from protein_inference_bayesian import infer_proteins_bayesian
    ...
    def test_posterior_probabilities_assigned(self):
        """Test that posterior probabilities are assigned to proteins."""
        from protein_inference_bayesian import infer_proteins_bayesian
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            infer_proteins_bayesian(input_path, output_path)
            ...
            for prot_id in protein_ids:
                for hit in prot_id.getHits():
                    ...
                    assert 0.0 <= score <= 1.0, (
                        f"Posterior probability {score} out of [0, 1] range"
                    )

    def test_both_proteins_present(self):
