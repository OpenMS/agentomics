...
Every script is a self-contained package under `tools/<domain>/<topic>/<tool_name>/`:

```
tools/proteomics/peptide_analysis/peptide_mass_calculator/
...
3. **Scaffold directory** — create `tools/<domain>/<topic>/<tool_name>/` with `requirements.txt`, empty `README.md`, empty test file
...
2. **Contribution requirements:**
   - Self-contained directory under `tools/<domain>/<topic>/<tool_name>/`
